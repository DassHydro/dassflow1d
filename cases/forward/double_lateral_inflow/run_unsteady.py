...
def run_unsteady():
    ...
    #------------------------------------------------------------------------------------------------------------------
    # Add inflow condition on segment 0 at coordinates (300.0, 0.0)
    ...
    model.add_inflow_condition(iseg=0, coords=[300.0, 0.0], t=[0, 3000.0], q=[20.0, 20.0])
    # Add inflow condition on segment 0 at coordinates (700.0, 0.0)
    # REMARK : when using DassFlow-1D v1.0 mesh files (old format, as for this case), the coordinates are the 2nd and 
    #          3rd values in the definition line of each cross-section
    model.add_inflow_condition(iseg=0, coords=[700.0, 0.0], t=[0, 3000.0], q=[20.0, 20.0])
